...
##############################################################################
effect.size<-function(e.size,samp.size){ 
  #weibull,effect size=0
  wei.var<-(80-e.size*80)
  x1<-rWEI(n=samp.size,mu=sqrt(wei.var/20),sigma=0.5) 
  level1<-x1+4-2*sqrt(wei.var/20)
  level2<-rWEI(n=samp.size,mu=2,sigma=0.5)
  #weibull,censor data,effect size=0
  x1<-rWEI(n=samp.size,mu=0.25*sqrt(wei.var/20),sigma=0.5) 
  cen1<-x1+4-2*sqrt(wei.var/20) 
  cen2<-rWEI(n=samp.size,mu=2,sigma=0.5)
  ...
  c<-c(c1,c2)
  g<-c(rep(1,samp.size),rep(2,samp.size))
  y<-c(y1,y2)
  ...
  t5<-gamlss(right~g,family=WEIrc) #alt hyp model
  t0<-gamlss(right~1,family=WEIrc) #null hyp model
  ...
  t6<-gamlss(right~g,sigma.formula=~g,family=WEIrc)
...
####################################################################################
NewSimu<-function(samp.size){
  a<-effect.size(0,samp.size)
  b<-effect.size(0.1,samp.size)
  c<-effect.size(0.2,samp.size)
  d<-effect.size(0.3,samp.size)
  e<-effect.size(0.5,samp.size)
  return(c(a,b,c,d,e))
}
all<-NewSimu(samp.size)
...
alpha<-0.05
effect<-c(0,0.1,0.2,0.3,0.5)
NewSimu(samp.size)
set.seed(1500)#gamma model
#set.seed(1500)#weibull model
#set.seed(1700)#LOGNO model

result<-matrix(NA,nrow=1000,ncol=25)
result
for(i in 1:100) result[i,]<-try(NewSimu(samp.size),silent=TRUE)
for(i in 101:200) result[i,]<-try(NewSimu(samp.size),silent=TRUE)
for(i in 201:300) result[i,]<-try(NewSimu(samp.size),silent=TRUE)
for(i in 301:400) result[i,]<-try(NewSimu(samp.size),silent=TRUE)
for(i in 401:500) result[i,]<-try(NewSimu(samp.size),silent=TRUE)
for(i in 501:600) result[i,]<-try(NewSimu(samp.size),silent=TRUE)
for(i in 601:700) result[i,]<-try(NewSimu(samp.size),silent=TRUE)
for(i in 701:800) result[i,]<-try(NewSimu(samp.size),silent=TRUE)
for(i in 801:900) result[i,]<-try(NewSimu(samp.size),silent=TRUE)
for(i in 901:1000) result[i,]<-try(NewSimu(samp.size),silent=TRUE)
...
title("0-0.5 weibull Model to Weibull Data, n=25, Unequal variances,unequal status")
