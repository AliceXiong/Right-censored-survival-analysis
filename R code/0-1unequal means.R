...
effect.size<-function(e.size,samp.size){ 
  ...
  t5<-gamlss(right~g,family=GArc) #alt hyp model
  t0<-gamlss(right~1,family=GArc) #null hyp model
  ...
  t6<-gamlss(right~g,sigma.formula=~g,family=GArc)
...
NewSimu<-function(samp.size){
  ...
  c<-effect.size(0.3,samp.size)
  d<-effect.size(0.5,samp.size)
  e<-effect.size(1,samp.size)
  return(c(a,b,c,d,e))
}
...
NewSimu(samp.size)
#set.seed(700)#gamma model
#set.seed(700)#weibull model
set.seed(600)#LOGNO model
...
title("0-1 GA Model to Weibull Data, n=25, Unequal mean")
