...
effect.size<-function(e.size,samp.size1,samp.size2){ 
  ...
  x1<-rWEI(n=samp.size1,mu=sqrt(wei.var/20),sigma=0.5) 
  level1<-x1+4-2*sqrt(wei.var/20)
  level2<-rWEI(n=samp.size2,mu=2,sigma=0.5)
  #weibull,censor data,effect size=0
  x1<-rWEI(n=samp.size1,mu=sqrt(wei.var/20),sigma=0.5) 
  cen1<-x1+4-2*sqrt(wei.var/20) 
  cen2<-rWEI(n=samp.size2,mu=2,sigma=0.5)
...
NewSimu<-function(samp.size1,samp.size2){
  ...
  c<-effect.size(0.2,samp.size1,samp.size2)
  d<-effect.size(0.3,samp.size1,samp.size2)
  e<-effect.size(0.5,samp.size1,samp.size2)
  return(c(a,b,c,d,e))
}
...
set.seed(1300)#gamma model
#set.seed(1200)#weibull model
#set.seed(1100)#LOGNO model
...
title("0-0.5 GA Model to Weibull Data, n=25,50, Unequal variances")
