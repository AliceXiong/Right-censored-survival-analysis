...
effect.size<-function(e.size,samp.size){ 
  ...
  mu<-5
  sigma<-0.5
  level1<-rWEI(n=samp.size,mu=5*(e.size)^2,sigma) 
  level2<-rWEI(n=samp.size,mu,sigma)
  #weibull,censor data,effect size=0
  cen1<-rWEI(n=samp.size,mu=5*(e.size)^2,sigma)  
  cen2<-rWEI(n=samp.size,mu,sigma)
...
NewSimu<-function(samp.size){
  a<-effect.size(1,samp.size)
  b<-effect.size(2,samp.size)
  c<-effect.size(3,samp.size)
  d<-effect.size(4,samp.size)
  e<-effect.size(5,samp.size)
  return(c(a,b,c,d,e))
}
...
effect<-c(1,2,3,4,5)
NewSimu(samp.size)
#set.seed(300)#gamma model
set.seed(400)#weibull model
#set.seed(500)#LOGNO model
...
title("1-5 weibull Model to Weibull Data, n=25, hazard rate")
